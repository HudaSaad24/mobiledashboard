...
if 'current_page' not in st.session_state:
    st.session_state.current_page = "Odyssey_Overview"
...
with col2:
    if st.session_state.current_page == "pages/Zeniva_Overview":
        button_style = "active-button"
    else:
        button_style = "default-button"
    if st.button("Zeniva", key="navigate_to_first"):
        st.switch_page("pages/Zeniva_Overview.py")
...
.button {{
    flex: 1; /* This makes each button take up 50% of the container */
    padding: 10px 20px;
    border: none;
    border-radius: 5px;
    font-size: 16px;
...
.button.active {{
    background-color: #F68C5B; /* Orange color */
    color: black;
    width: 100%; /* Make the active button span the full width of the container */
}}
